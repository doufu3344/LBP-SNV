...
for i = 1:info.ncls
    ...
    for j = 1:info.nsbj
        ...
        for k = 1:info.nemp
            ...
            lbpnormalName = [info.lbppath, '\', idxcls, '_', idxsbj, '_', idxemp, '_lbp.mat'];
            
            % some missed videos
            if ~exist(lbpnormalName, 'file')
                continue;
            end
            
            load(lbpnormalName, 'lbp', 'masks');           
            [pns, rows, cols, frms] = getPolyLBP(lbp, scheme.lx, scheme.ly, scheme.lt);
            clear lbp;
